...
Figure 1 shows the proportion of cooperators that emerges for different values of T, but it does not show how the networks reached their final conditions. In order to better understand the process by which the networks change over time, we graph the proportion of cooperators, %C, in a network over time. We begin with a network with an initial cooperator ratio, c<sub>0</sub>, and graph the value of %C at each time step. Since we can divide the following 3 regimes, we chose the T value(T = 1.1, 1.7, 3.0) in each regime.
...
We could think that the initial population of cooperators is the major factor as well as T value. If a cooperator is among defectors, it will always choose to be a defector. So cooperators must be clustered at the beginning in order to dominate the network. However, when the initial population of cooperators is low, many of cooperators are surrounded by defectors and defectors eventually dominate in the population.
